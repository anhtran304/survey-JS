...
function loadQuestions() {
    ...
    a.open("GET", "https://api.myjson.com/bins/9l899", true);
  ...
  if (page >= 0) {
    if (myObj.length < (page + 1)) {
      ...
      for (var item in myObj) {
        ...
        holderHTML += '<div class="col-sm-12">' + myObj[item].question + ' ' + myObj[item].answers[myQueRep[item]] + ' ' + gylph + '</div>';
      }
...
function myAnswer() {
  ...
  for (var x = 0; x < bAnswer.length; x++) {
    ...
    console.dir(bAnswer[x].classList);
  }
